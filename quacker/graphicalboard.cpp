...
#if defined(Q_WS_WIN)
	PixmapCacher::self()->tileFont.setFamily(QString("Helvetica"));
#endif // Q_WS_WIN
...
PixmapCacher::PixmapCacher()
{
	...
	britishLetterColor = letterColor;
...
QColor TileWidget::tileColor(const Quackle::Board::TileInformation &information)
{
	...
	case Quackle::Board::LetterTile:
		if (information.isBritish && QuackerSettings::self()->britishColoring == TileBritishColoring)
...
QFont TileWidget::letterFont()
{
	...
	if (m_information.isStartLocation && m_information.tileType != Quackle::Board::LetterTile)
		return scaledFont(s_defaultLetterScale);
	...
	{
		double initialScale = s_defaultLetterScale;
...
QPixmap TileWidget::generateTilePixmap()
{
	...
	//UVcout << "currentSize: " << currentSize.width() << ", " << currentSize.height() << endl;

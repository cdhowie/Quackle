...
LetterString Board::prettyTilesOfMove(const Move &move, bool markPlayThruTiles) const
{
	...
	if (!markPlayThruTiles || move.action != Move::Place)
		return ret;

	// combine parens
	LetterString undoubledRet;

	const LetterString::const_iterator retEnd(ret.end());
	for (LetterString::const_iterator it = ret.begin(); it != retEnd; ++it)
	{
		if (*it == QUACKLE_PLAYTHRU_END_MARK)
		{
			++it;

			if (it == retEnd)
			{
				undoubledRet += QUACKLE_PLAYTHRU_END_MARK;
				break;
			}
			else
			{
				if (*it == QUACKLE_PLAYTHRU_START_MARK)
					continue;
				else
					undoubledRet += QUACKLE_PLAYTHRU_END_MARK;
			}

...
void Generator::spit(int i, const LetterString &prefix, int flags)
{
	...
	if (!(flags & ClearBlanknesses && m_counts[(int)(c)] >= 1))
	{
		if (m_counts[(int)(QUACKLE_BLANK_MARK)] >= 1 || flags & AddAnyLetters)
		{
			m_counts[(int)(QUACKLE_BLANK_MARK)]--;
...
void Generator::gaddagAnagram(const GaddagNode *node, const LetterString &prefix, int flags)
{
	...
	if (m_counts[QUACKLE_BLANK_MARK] >= 1 || flags & AddAnyLetters) {
		for (const GaddagNode* child = node->firstChild(); child; child = child->nextSibling()) {
			...
			if (flags & ClearBlanknesses && m_counts[(int)(childLetter)] >= 1) {
				continue;
			}
...
Move Generator::anagram()
{
	...
	vector<LetterString>::const_iterator end = m_spat.end();
	for (vector<LetterString>::const_iterator it = m_spat.begin(); it != end; ++it)
	{
...
{
	vector<LetterString> results = anagramLetters(word);
	
	vector<LetterString>::const_iterator end = results.end();
	for (vector<LetterString>::const_iterator it = results.begin(); it != end; ++it)
		if ((*it) == word)
			return true;
...
vector<LetterString> Generator::anagramLetters(const LetterString &letters, int flags)
{
